...
本插件提供 `ncqq-dashboard` Page，用于在 AstrBot WebUI 中查看多面板状态并处理审批。页面不替代聊天 workflow，也不编辑插件配置。
...
| `pages/ncqq-dashboard/style.css` | 页面样式，跟随 AstrBot 亮暗主题。 |

页面可在无 bridge 环境下使用预览数据渲染，便于本地检查布局。正式运行时只通过 bridge 请求后端。
